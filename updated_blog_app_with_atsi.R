...
denominators <- read_csv("my_denominators.csv") %>% filter(`Age (years)` == 0)
yearly_denominators <- denominators %>% 
  group_by(Year) %>% summarise(population = sum(adjusted_population))
...
server <- (function(input, output) {
  ...
  output$figure <- renderPlotly({
    ...
    if (option == "Figure 1: Safe sleeping risks over time"){
      ...
      # Yearly deaths
      totals <- sudi_sleeping %>% 
        left_join(select(sudi_sleeping, c("Case Number", "Year")), on = Year) %>% 
    ...
    # Figure three code
      ...
      fig5_data <- atsi_deaths %>% 
        ...
        mutate(`Cultural Background` = recode(`Cultural Background`,
                                              ATSI = "Aboriginal"))
      Year <- seq(2005, 2020)
      mean <- rep(round(mean(fig5_data[fig5_data$`Cultural Background`=="Aboriginal", "Rate", drop=TRUE]),2), 16)
      mean_df <- data.frame(Year, mean)
      
      fig5 <- plot_ly(data = fig5_data,
                      type = "scatter",
                      mode = "lines",
      ...
      add_trace(p = fig5,
                ...
                showlegend = FALSE,
                text = ~mean,
                hovertemplate = paste0("Median number of deaths: ", "%{text}"))
    }
